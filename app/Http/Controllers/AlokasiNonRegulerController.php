...
class AlokasiNonRegulerController extends Controller
{
    //
    ...
    public function index(Request $request)
	{
	   if (Auth::guest()) {
	    ...
	    return view('alokasinonreguler.index', []);
	}
	...
	public function create(Request $request)
	{
	    ...
	    return view('alokasinonreguler.add', [
	        []
	    ]);
	}
...
/*		$kamars = null;
		...
		$kamarNonReguler = KamarNonReguler::where('id_pendaftaran_non_reguler', $id)->orderBy('created_at', 'desc')->get()->first();	
		
	    return view('alokasinonreguler.add', [
	        'model' => $alokasi,
			'kamars' => $kamars,
			'kamarNonReguler' => $kamarNonReguler,
		]);
	}
	...
	public function show(Request $request, $id)
	{
		...
		$alokasi = AlokasiNonReguler::findOrFail($id);
	    return view('alokasinonreguler.show', [
	        'model' => $alokasi	    ]);
	}
	...
	public function grid(Request $request)
	{
		...
		if($user->is_admin == '1') {		
			$select = "SELECT id_daftar, id_user, nama, nomor_identitas, jenis_identitas, jenis_kelamin, asrama, tanggal_masuk, tanggal_keluar, nama_kamar ";
			$presql = " FROM 
							(SELECT * FROM (SELECT id_daftar, id_user, nama, nomor_identitas, jenis_identitas, jenis_kelamin, asrama, tanggal_masuk, tanggal_keluar 
								FROM users JOIN daftar_asrama_non_reguler WHERE id = id_user AND (status = 'Diterima') AND DATE(NOW()) <= tanggal_keluar) b 
								LEFT JOIN 
	...
	public function grid2(Request $request)
	{
		...
		if($user->is_admin == '1') {		
			...
			$presql = " FROM 
							(SELECT * FROM (SELECT id_daftar, id_user, nama, nomor_identitas, jenis_identitas, jenis_kelamin, asrama, tanggal_masuk, tanggal_keluar 
								FROM users JOIN daftar_asrama_non_reguler WHERE id = id_user AND (status = 'Teralokasi') AND DATE(NOW()) <= tanggal_keluar) b 
								LEFT JOIN 
									(SELECT d.id_pendaftaran_non_reguler, created, d.id_kamar, e.nama AS nama_kamar
										FROM
										(
			...
			$presql = " FROM 
							(SELECT * FROM (SELECT id_daftar, id_user, nama, nomor_identitas, jenis_identitas, jenis_kelamin, asrama, tanggal_masuk, tanggal_keluar 
								FROM users JOIN daftar_asrama_non_reguler WHERE id = id_user AND (status = 'Teralokasi') AND DATE(NOW()) <= tanggal_keluar AND asrama = '".$asrama[0]->nama."') b 
								LEFT JOIN 
									(SELECT d.id_pendaftaran_non_reguler, created, d.id_kamar, e.nama AS nama_kamar
										FROM
										(
	...
	public function update(Request $request) {
		$kamarNonRegulerLama = null;
		$kamarNonRegulerBaru = null;
		$idKamarLama = null;
		$kamarLama = null;
		$idKamarBaru = null;
		$kamarBaru = null;
		...
		$kamarNonRegulerLama = KamarNonReguler::where('id_pendaftaran_non_reguler', $request->id_pendaftaran_non_reguler)->orderBy('created_at', 'desc')->get()->first();
		$alokasi = AlokasiNonReguler::findOrFail($request->id_pendaftaran_non_reguler);
		if($kamarNonRegulerLama != null) {
			$idKamarLama = $kamarNonRegulerLama->id_kamar;
			if($request->tanggal_masuk > date("Y-m-d")) { // tanggal sekarang lebih awal dibanding tanggal masuk
				$kamarNonRegulerBaru = KamarNonReguler::where('id_pendaftaran_non_reguler', $request->id_pendaftaran_non_reguler)->orderBy('created_at', 'desc')->get()->first();
			} else {
				$kamarNonRegulerBaru = new KamarNonReguler;	
			};
		} else {
			$kamarNonRegulerBaru = new KamarNonReguler;
		}
		...
		if(!($idKamarLama == $idKamarBaru)) {		
			$kamarNonRegulerBaru->id_pendaftaran_non_reguler = $request->id_pendaftaran_non_reguler;
			$kamarNonRegulerBaru->id_kamar = $request->id_kamar;
			if($idKamarLama == null || ($request->tanggal_masuk > date("Y-m-d"))) {
				$kamarNonRegulerBaru->tanggal_awal = $request->tanggal_masuk;
			} else {
				$kamarNonRegulerBaru->tanggal_awal = date("Y-m-d");
			}
			$kamarNonRegulerBaru->tanggal_akhir =  $request->tanggal_keluar;
			
			if($idKamarLama != null) {
				if(!($request->tanggal_masuk > date("Y-m-d"))) $kamarNonRegulerLama->tanggal_akhir = date("Y-m-d");
			...
			/*	if ($idKamarLama != null) $kamarLama->save();
				...
				if($kamarNonRegulerLama == null) {
					...
					$kamarNonRegulerLama->save();
				}
				if($kamarNonRegulerBaru->id_kamar == null) {
				
				} else {
					$kamarNonRegulerBaru->save();
				}
		...
		if(!$valid) {
			return redirect('/alokasinonreguler')->with('message', 'Gagal mengalokasikan kamar! Kamar penuh.');
		} else {
			return redirect('/alokasinonreguler');
		}
