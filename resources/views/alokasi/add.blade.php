...
<div class="container">
	<div class="panel panel-default">
		<div class="panel-heading">
			<h2>Atur Alokasi Kamar</h2>
		</div>
		<div class="panel-body">
<!-- {{ $kamars }} -->
			<form action="{{ url('/alokasi'.( isset($kamarReguler) ? "/" . $model->id : "")) }}" method="POST" class="form-horizontal">
				{{ csrf_field() }}

				@if (isset($kamarReguler))
					<input type="hidden" name="_method" value="PATCH">
				@endif


				<div class="form-group" style="display: none;">
					<label for="id_pendaftaran_reguler" class="col-sm-4 control-label">Id daftar</label>
					<div class="col-sm-6">
						<input type="text" name="id_pendaftaran_reguler" id="id_pendaftaran_reguler" class="form-control" value="{{$model['id_daftar'] or ''}}" readonly="readonly">
					</div>
				</div>
				...
				<div class="form-group">
					<label for="nama_periode" class="col-sm-4 control-label">Periode</label>
					<div class="col-sm-6">
						<input type="text" name="nama_periode" id="nama_periode" class="form-control" value="{{$model['nama_periode'] or ''}}" readonly="readonly">
					</div>
				</div>	
						...
						<div class="col-sm-6">
							<select id="id_kamar" class="form-control" name="id_kamar">
								@foreach ($kamars as $kamar)
									@if ($kamar->gender == $model->jenis_kelamin && $kamar->status == "Reguler" && ($kamar->jumlah_penghuni ? $kamar->jumlah_penghuni : 0) < $kamar->kapasitas)
										<option
										@if ($kamarReguler != null && ($kamar->id_kamar == $kamarReguler->id_kamar))
											selected
										@endif
